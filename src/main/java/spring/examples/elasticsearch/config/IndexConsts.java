package spring.examples.elasticsearch.config;

public class IndexConsts {

    public final static String PRODUCTS_INDEX_NAME = "products";
    public final static String PRODUCTS_CATEGORY_SUFFIX = "catsuffix";

    public static final String PRODUCT_FIELD_NAME  = "name";
    public static final String PRODUCT_FIELD_CATEGORY  = "category";
    public static final String PRODUCT_FIELD_MANUFACTURER  = "manufacturer";
}
